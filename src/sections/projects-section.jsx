...
const PhoneMockup = React.memo(({ project, isActive }) => {
  return (
    <div className={`relative will-change-transform transition-all duration-700 scale-70`}>
      {/* iPhone Frame with Enhanced Realism */}
      <div className="relative mx-auto w-72 h-[580px]">
        
        {/* Enhanced Glow Effect */}
        {isActive && (
          <div className={`absolute -inset-8 bg-gradient-to-r ${project.color} opacity-20 blur-2xl rounded-full pointer-events-none animate-pulse`}></div>
        )}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-gray-700 via-gray-800 to-gray-900 rounded-[3.5rem] shadow-2xl border-4 border-gray-600">
          ...
          <div className="absolute inset-1 bg-gradient-to-b from-gray-800 to-black rounded-[3rem] shadow-inner">
            ...
            <div className="absolute top-3 left-3 right-3 bottom-3 bg-black rounded-[2.8rem] overflow-hidden shadow-inner border border-gray-900">
              ...
              {/* Screen Content with Realistic iOS Elements */}
              <div className="relative h-full bg-gradient-to-b from-gray-900 via-black to-gray-900 p-4 pt-12">
                
                {/* Enhanced Status Bar */}
                <div className="flex justify-between items-center text-white text-xs mb-6 px-2">
                  <div className="flex items-center gap-1">
                    <span className="font-semibold">9:41</span>
                    <div className="w-0.5 h-3 bg-white opacity-40 ml-1"></div>
                  </div>
                  <div className="flex items-center gap-1">
                    {/* Signal Bars */}
                    <div className="flex gap-0.5">
                      {[1,2,3,4].map(i => (
                        <div key={i} className={`w-1 bg-white rounded-full ${i <= 3 ? 'h-2' : 'h-1.5 opacity-40'}`}></div>
                      ))}
                    </div>
                    {/* WiFi */}
                ...
                {/* App Interface with iOS Design Language */}
                <div className="h-full relative">
                  
                  {/* App Header with Glassmorphism */}
                  <div className={`relative w-full h-44 bg-gradient-to-br ${project.color} rounded-3xl mb-4 overflow-hidden shadow-xl`}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-900 bg-opacity-80 backdrop-blur-lg rounded-2xl p-4 border border-gray-800 shadow-lg">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="text-white font-semibold text-sm">{project.category}</h3>
                          <p className="text-gray-400 text-xs mt-1">Development Project</p>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {project.features.slice(0, 2).map((feature, i) => (
                          <span key={i} className="text-xs bg-gradient-to-r from-gray-800 to-gray-700 text-gray-200 px-3 py-1.5 rounded-xl border border-gray-600 shadow-sm">
                            {feature}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex gap-3">
                        <button className="flex-1 py-3 bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl text-xs font-semibold text-white shadow-lg active:scale-95 transition-transform">
                          <div className="flex items-center justify-center gap-2">
                            <span>▶</span>
                            <span>Demo</span>
                          </div>
                        </button>
                        <button className="px-4 py-3 bg-gradient-to-r from-gray-800 to-gray-700 rounded-xl text-white text-xs font-medium border border-gray-600 shadow-lg active:scale-95 transition-transform">
                          <span>{'</>'}</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-900 bg-opacity-60 backdrop-blur-lg rounded-2xl p-3 border border-gray-800">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${project.color} shadow-lg`}></div>
                          <span className="text-gray-300 text-xs">Live Project</span>
                        </div>
                ...
                {/* Home Indicator */}
                <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-white opacity-60 rounded-full"></div>
              </div>
            </div>
          </div>
        ...
        {isActive && (
          <>
            <div className={`absolute -inset-2 bg-gradient-to-r ${project.color} opacity-5 blur-xl rounded-[4rem] pointer-events-none`}></div>
...
const DesktopMockup = React.memo(({ project, isActive }) => {
  return (
    <div className={`relative will-change-transform transition-transform duration-500 ${isActive ? 'scale-105' : 'scale-95 opacity-70'}`}>
      {/* MacBook Frame */}
      <div className="relative mx-auto w-96 h-64">
        ...
        <div className="relative w-full h-48 bg-gray-900 rounded-t-lg overflow-hidden shadow-xl border border-gray-700">
          ...
          <div className="h-full bg-gray-900 p-4">
            {/* Header */}
            <div className={`w-full h-12 bg-gradient-to-r ${project.color} rounded-lg mb-3 flex items-center justify-center`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className={`h-16 bg-gradient-to-r ${project.color} opacity-30 rounded-lg`}></div>
            </div>
          </div>
        </div>
        ...
        {isActive && (
          <div className={`absolute -inset-6 bg-gradient-to-r ${project.color} opacity-10 blur-lg rounded-lg pointer-events-none`}></div>
        )}
      </div>
    </div>
  );
        ...
        <ul className="space-y-2">
          {project.features.map((feature, i) => (
            <li key={i} className="text-gray-300 flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${project.color} flex-shrink-0`}></div>
              {feature}
            </li>
          ))}
        </ul>
...
function ProjectsSection() {
  ...
  }, [isVisible, filteredProjects.length]);
  ...
  return (
    <div id="projects" className="relative z-50 border-t my-12 lg:my-24 border-[#25213b]">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mt-16">
          <h3 className="text-2xl font-bold text-white text-center mb-8">All Projects Overview</h3>
            ...
            {filteredProjects.map((project, index) => (
              <div
                ...
                className={`cursor-pointer bg-black-100 border border-gray-700 rounded-xl p-4 transition-all duration-300 hover:border-pink-500/50 hover:scale-105 ${
                  activeProject === index ? 'border-pink-500/50 shadow-lg shadow-pink-500/10' : ''
                }`}
              >
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-xs px-2 py-1 rounded text-white`} style={{ backgroundColor: project.accent }}>
                    {project.status}
                  </span>
                </div>
              </div>
